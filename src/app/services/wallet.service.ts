...
export class WalletService {
  ...
  decrypt(passphrase: string, walletEnckey: string): Observable<boolean> {
    ...
    this.syncWallet(selectedWalletId, passphrase, walletEnckey).subscribe(
      (data) => {
        console.log(`sync wallet result=${JSON.stringify(data)}`);
  ...
  sendToAddress(
    ...
    console.log(`send enckey ${enckey}  passphrase=${passphrase}`);
    console.log("sendToAddress viewkeys=", JSON.stringify(viewKeys));
    return this.http.post<string>(this.coreUrl, {
      jsonrpc: "2.0",
      id: "jsonrpc",
